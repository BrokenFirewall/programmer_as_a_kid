...
public class ascii {
	...
	ascii(){
		finestra=new JFrame();
		finestra.setTitle("Cifrario monoalfabetico esteso a tutto Ascii");
		...
		c.addKeyListener(new KeyListener(){ 
			...
			public void keyReleased(KeyEvent e){	
				if((Integer.parseInt(c.getText())>223)||(Integer.parseInt(c.getText())<1))
				    {c1.setText("ERRORE");
			...
			public void actionPerformed(ActionEvent e){
				...
				for(int i=0;i<x;i++)
					if((tch[i]+kk)<=255)
				    	...
				    	{codice=ch.codePointAt(i)+kk-223;
				    	 cifrato.setText(cifrato.getText()+(char)codice);}
						
				for(int i=0;i<x;i++)
				if((tch[i]<32)||(tch[i]>255))
...
//metà dell'algoritmo
		...
		dc=new JTextField("30");
		dc.addKeyListener(new KeyListener(){ 
			public void keyPressed(KeyEvent e){
			}
				
			public void keyReleased(KeyEvent e){	
				if((Integer.parseInt(dc.getText())>223)||(Integer.parseInt(dc.getText())<1))
				    {c2.setText("ERRORE");
			...
			public void actionPerformed(ActionEvent e){
				...
				for(int i=0;i<x2;i++)
					if((tci2[i]-kk2)>=32)
						...
						{codice2=ci2.codePointAt(i)-kk2+223;
						 dchiaro.setText(dchiaro.getText()+(char)codice2);}
				
				for(int i=0;i<x2;i++)
				if((tci2[i]<32)||(tci2[i]>255))
